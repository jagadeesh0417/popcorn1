...
import { CreditCard, MapPin, User, MessageSquare, Lock, ShoppingBag, Store } from "lucide-react";
...
export default function CheckoutPage() {
  ...
  const handlePayment = async () => {
    ...
    if (shippingMethod === "courier" && (!form.address || !form.city || !form.state || !form.zipCode)) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid lg:grid-cols-5 gap-10">
          <div className="lg:col-span-3 space-y-6">
            <div className="bg-[#FFF8F0] p-6 border border-[rgba(183,28,28,0.08)]">
              <div className="flex items-center gap-2 mb-5">
                <Store className="h-5 w-5 text-[#B71C1C]" />
                <h2 className="font-bold text-lg text-[#1A1A1A]">Shipping Method</h2>
              </div>
              <div className="space-y-3">
                <label className={`flex items-center gap-4 p-4 border cursor-pointer transition-colors ${shippingMethod === "courier" ? "border-[#B71C1C] bg-white" : "border-[rgba(183,28,28,0.12)] bg-white"}`}>
                  <input type="radio" name="shipping" value="courier" checked={shippingMethod === "courier"} onChange={() => setShippingMethod("courier")} className="accent-[#B71C1C]" />
                  <div>
                    <p className="font-medium text-sm text-[#1A1A1A]">Pan-India Shipping</p>
                    <p className="text-xs text-[#666666]">Delivered via India Post. 3-7 days.</p>
                  </div>
                </label>
                <label className={`flex items-center gap-4 p-4 border cursor-pointer transition-colors ${shippingMethod === "pickup" ? "border-[#B71C1C] bg-white" : "border-[rgba(183,28,28,0.12)] bg-white"}`}>
                  <input type="radio" name="shipping" value="pickup" checked={shippingMethod === "pickup"} onChange={() => setShippingMethod("pickup")} className="accent-[#B71C1C]" />
                  <div>
                    <p className="font-medium text-sm text-[#1A1A1A]">Mysuru Pickup — Free</p>
                    <p className="text-xs text-[#666666]">Pick up from our kitchen in Vijayanagar 4th Stage.</p>
                  </div>
                </label>
              </div>
            </div>
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                {[
                  ...
                  <div key={field.id} className="space-y-2">
                    ...
                    <Input id={field.id} type={field.type || "text"} value={(form as Record<string, string>)[field.id]} onChange={(e) => updateField(field.id, e.target.value)} placeholder={field.placeholder} className="bg-white border-[rgba(183,28,28,0.12)]" />
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="bg-[#FFF8F0] p-6 border border-[rgba(183,28,28,0.08)]">
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Textarea id="address" value={form.address} onChange={(e) => updateField("address", e.target.value)} placeholder="Street, building, apartment" className="bg-white border-[rgba(183,28,28,0.12)] min-h-[80px]" />
                  </div>
                  <div className="grid sm:grid-cols-3 gap-4">
                    {[
                      ...
                      <div key={field.id} className="space-y-2">
                        <Label htmlFor={field.id} className="text-[#1A1A1A]">{field.label}</Label>
                        <Input id={field.id} value={(form as Record<string, string>)[field.id]} onChange={(e) => updateField(field.id, e.target.value)} placeholder={field.placeholder} className="bg-white border-[rgba(183,28,28,0.12)]" />
                      </div>
                    ))}
                  </div>
                </div>
            ...
            {shippingMethod === "pickup" && (
              <div className="bg-[#FFF8F0] p-6 border border-[rgba(183,28,28,0.08)]">
                ...
                <p className="text-sm text-[#666666]">
                  Poprika Kitchen<br />
                  Vijayanagar 4th Stage, Mysuru<br />
            ...
            <div className="bg-[#FFF8F0] p-6 border border-[rgba(183,28,28,0.08)]">
              <div className="flex items-center gap-2 mb-5">
                ...
                <h2 className="font-bold text-lg text-[#1A1A1A]">{shippingMethod === "pickup" ? "Pickup Notes" : "Delivery Instructions"}</h2>
              </div>
              <Textarea value={form.deliveryInstructions} onChange={(e) => updateField("deliveryInstructions", e.target.value)} placeholder={shippingMethod === "pickup" ? "Preferred pickup time, anything we should know..." : "Gate code, landmark, preferred delivery time..."} className="bg-white border-[rgba(183,28,28,0.12)] min-h-[80px]" />
            </div>
            ...
            <div className="bg-[#FFF8F0] p-6 border border-[rgba(183,28,28,0.08)]">
              ...
              <RadioGroup defaultValue="razorpay">
                <div className="flex items-center gap-3 p-4 bg-white border border-[rgba(183,28,28,0.12)]">
                  <RadioGroupItem value="razorpay" id="razorpay" />
                  <Label htmlFor="razorpay" className="flex items-center gap-3 cursor-pointer flex-1">
                    <div className="w-10 h-7 bg-[#B71C1C]/10 flex items-center justify-center text-xs font-bold text-[#B71C1C]">RZP</div>
                    <div>
          ...
          <div className="lg:col-span-2">
            <div className="bg-[#FFF8F0] p-6 border border-[rgba(183,28,28,0.08)] sticky top-28">
              ...
              <Button className="w-full mt-6 bg-[#B71C1C] hover:bg-[#8E1414] text-white h-12 text-base shadow-lg shadow-[#B71C1C]/20" onClick={handlePayment} disabled={loading}>
                {loading ? (
